...
$(function(){
	...
	var spell_tmpl = '<div class="spell" data-attr="{identifier}"><div class="spell-content"><img src="/wizards/img/spell{i}.png" /><h3>{name}</h3></div></div>';
	...
	var wizLoad = $.ajax({
		url: 'http://creator3.monkeydancers.com/api/v1/game_objects/c172553', 
		type: 'GET', 
		dataType: 'json', 
		data: {
			key: "d594a2e074eed72b9f0612efb0d2660cd3ee1893"
		}
	}); 

	var spellLoad = $.ajax({
		url: 'http://creator3.monkeydancers.com/api/v1/game_objects', 
		type: 'GET', 
		dataType: 'json', 
		data: {
			key: "d594a2e074eed72b9f0612efb0d2660cd3ee1893", 
	...
	var cast = function(e){
		...
		var b = $("body"); 
		b.addClass('loading');
		var spell = window.spells[window.swipe.getPos()]; 
		var p = $.ajax({
			url: 'http://creator3.monkeydancers.com/trigger', 
			type: 'POST', 
			dataType: 'json', 
			data: {
				apikey: "d594a2e074eed72b9f0612efb0d2660cd3ee1893", 
				target: spell.identifier, 
				actor: window.wizard.identifier
			}
		}); 
		$.when(p).then(function(data){
			...
			$(".mana").html(data.mana);
			$(".gold").html(data.gold);
			notify(data.spell_cast);
			b.removeClass('loading');
		})
	}
	
	$.when(wizLoad, spellLoad).then(function(wizard, spells){
		wizard = wizard[0].object; 
		window.spells = spells[0];
		window.wizard = wizard;

		$(".mana").html(wizard['mana'].value);
		$(".gold").html(wizard['gold'].value);

		var sw = $(".swipe-wrap"); 
		$.each(window.spells, function(s, v){
			v.i = s+1; 
			sw.append($(spell_tmpl.supplant(v))); 
		});
		
		setTimeout(function(){
			window.swipe = Swipe(document.getElementById('slider'));}, 200);

		$(".spell-cast-btn").on('tap', cast); 
